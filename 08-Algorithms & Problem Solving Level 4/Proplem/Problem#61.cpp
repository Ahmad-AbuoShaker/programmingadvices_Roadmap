...
bool isDateInPeriods(stPeriod Period1, sDate Date) {
    return !(CompareDates(Period1.StartDate, Date) == After
        ||
        CompareDates(Period1.EndDate, Date) == Before);

}

void swap(stPeriod& Period1, stPeriod& Period2) {
...
int GetOverlapDays(stPeriod Period1, stPeriod Period2) {
    ...
    if (CompareDates(Period1.StartDate, Period2.StartDate) == After)
    {
        swap(Period1, Period2);
    }
    
    if (CompareDates(Period2.EndDate, Period1.EndDate) == After) {
...
int CounterOverlapDays(stPeriod Period1, stPeriod Period2) {
    if (!IsOverlapPeriods(Period1, Period2))
        return 0;
    ...
    if (Period1Length < Period2Length) {
        while (IsDate1BeforeDate2(Period1.StartDate, Period1.EndDate)) {
            if (isDateInPeriods(Period2, Period1.StartDate))
                OverlapDays++;
        ...
        while (IsDate1BeforeDate2(Period2.StartDate, Period2.EndDate)) {
            if (isDateInPeriods(Period1, Period2.StartDate))
                OverlapDays++;
